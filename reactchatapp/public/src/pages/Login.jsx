...
import { loginRoute, meRoute, newLoginRoute } from '../utils/APIRoutes';
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
   ...
   if( handleValidation()){
    ...
    //   password
    // });
    const verifyData = await axios.post(newLoginRoute, {
      phone: username,
      otp: password
    });
    localStorage.setItem('access-token', verifyData.data.data.accessToken);
    const {data} = await axios.get(meRoute, {
      headers: {
        'Authorization': `Bearer ${verifyData.data.data.accessToken}`
      }
    });
    console.log(data);
    if(data.status === "fail"){
